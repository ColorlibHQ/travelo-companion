...
function travelo_companion_frontend_scripts() {
	...
	wp_enqueue_script( 'jquery-ui-datepicker', '', array( 'jquery' ), '1.11.4', true  );
...
function set_custom_edit_listing_columns($columns) {
    ...
    $columns['listing_price'] = __( 'Listing Price', 'travelo_companion' );
    $columns['listing_area'] = __( 'Listing Area', 'travelo_companion' );
...
function custom_listing_column( $column, $post_id ) {
    ...
    switch ( $column ) {
        ...
        case 'listing_price' :
            echo get_post_meta( $post_id , '_travelo_listing_price' , true ); 
            break;

        case 'listing_area' :
            echo get_post_meta( $post_id , '_travelo_listing_area' , true ); 
            break;
...
if ( ! function_exists( 'search_prop_form_datas' ) ) {
	function search_prop_form_datas() {
		...
		if ( $search_text == '' || $search_category == '' || $search_location == '' || $search_area_from == '' || $search_area_to == '' || $price_min == '' || $price_max == '' ) {
			...
			$response['message']  = __( 'Sorry! Empty field is not allowed.', 'travelo-companion' );
		} else {
			$item = 0;
